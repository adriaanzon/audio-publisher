...
class ProcessingJob:
    ...
    @classmethod
    def from_source(
        cls, source_bucket: str, source_object: str, destination_bucket: str
    ):
...
def check_processing_status(
    job: ProcessingJob, storage_client: storage.Client
) -> ProcessingStatus:
    ...
    if output_blob.exists():
        print(
            f"Output already exists: gs://{job.destination_bucket}/{job.output_filename} - skipping processing"
        )
        return ProcessingStatus.ALREADY_COMPLETE

    if placeholder_blob.exists():
        print(
            f"Processing already in progress: gs://{job.destination_bucket}/{job.placeholder_filename} - skipping duplicate"
        )
...
def create_processing_placeholder(job: ProcessingJob, storage_client: storage.Client):
    ...
    print(
        f"Creating processing placeholder: gs://{job.destination_bucket}/{job.placeholder_filename}"
    )
    placeholder_data = {
        "status": "processing",
...
def mark_processing_failed(
    job: ProcessingJob, error_code: str, storage_client: storage.Client
):
    ...
    placeholder_blob.upload_from_string(
        json.dumps(error_data), content_type="application/json"
    )
    print(
        f"Updated placeholder with error status: gs://{job.destination_bucket}/{job.placeholder_filename}"
    )


def download_source_file(
    job: ProcessingJob, destination_path: Path, storage_client: storage.Client
):
    """Download the source WAV file from GCS to local disk."""
    print(
        f"Downloading gs://{job.source_bucket}/{job.source_object} to {destination_path}"
    )
...
def upload_result_and_cleanup(
    job: ProcessingJob, output_path: Path, storage_client: storage.Client
):
    ...
    output_blob = bucket.blob(job.output_filename)
    print(
        f"Uploading {output_path} to gs://{job.destination_bucket}/{job.output_filename}"
    )
    ...
    print(
        f"Successfully normalized and uploaded: gs://{job.destination_bucket}/{job.output_filename}"
    )
...
def normalize(source_bucket: str, source_object: str, destination_bucket: str) -> str:
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        if source_path.stat().st_size == 0:
            print(
                f"ERROR: Source file is 0 bytes (corrupted): gs://{source_bucket}/{source_object}"
            )
